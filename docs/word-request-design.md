...
| 5（任意） | 第2層＝同梱辞書のインデックス化（日本語コース対応） | 不要・0円のはずだった | ⏭️ 見送り（2026-08-02・Kohei判断） |

**Phase 0〜4 完了・Phase 5 は見送り。** 検索・静的プール・AI生成・管理画面まで一通り実用になっている。

### Phase 5 を見送った理由（2026-08-02 調査結果）

`pipeline/raw/` に JMdict（117MB）・EJDict・CMUdict・Tatoeba は実は既に全部揃っており、新規調達は不要だった。
しかし調査の結果、当初の想定（「英語コースの静的プールと同じ0円レイヤーを日本語コースにも」）には
2つの誤算があると判明した：

1. **訳語の言語ミスマッチ**：JMdict／kaikki-tagalog の訳語は英語のみ。日本語コース（`ja-*`）が
   必要とするのはタガログ語訳、タガログコース（`tl-0-2k`）が必要とするのは日本語訳——どちらも
   同梱辞書の訳語とは言語が違う。ゼロ円の辞書引きでは正しい言語のカードを作れない
2. **「残りプール」が僅少**：英語コースは3万語生成→1.2万語採用で8,324語の未使用プールが残ったが、
   日本語コースは生成した分をほぼ全部採用済み（`ja-3-10k`: 残り0語・`ja-10-30k`: 残り約114語）。
   JMdictの一般語フラグ付き語で未収録なのは3コース合計で約1,122語のみ（英語の8,324語の約1/7）

投資対効果が薄いと判断し見送り。**Phase 3（AI生成）が既に同じ穴を埋めている**——
Phase 3 で見つかった言語ハードコードのバグ（下記）を直したことで、`ja-*`・`tl-0-2k` でも
正しい言語のAI生成が動くようになった。将来 Phase 5 をやるなら、残りプールの一括AI翻訳
（0円ではなくなる）か、辞書自体が大きいタガログコース（kaikki 3.2万語）限定が候補。

---

## 13. AI単語生成の対象言語バグ（2026-08-02・Phase 3 完了後に発覚・修正済み）

Phase 3 完了直後、Phase 5 の調査中に **`worker/wordgen.ts` のプロンプトが「English headword→
Japanese gloss」に決め打ちされていた**ことが判明した。`en-10-30k`（学習言語=英語・訳言語=日本語）
では偶然正しく動いていたが、`ja-*`（学習言語=日本語・訳言語=タガログ語）や `tl-0-2k`（学習言語=
タガログ語・訳言語=日本語）で使うと誤った言語のカードが生成され得た——`ja-*` コースでローマ字
検索した場合や、`tl-0-2k` の単語検索で実際に到達可能なコードパスだった。

**修正内容**：
- `learningLanguage`/`glossLanguage` をコースの `meta.json`（`learningLanguage`/`glossLanguage`
  フィールド）から動的に取得し、`buildGeneratePrompt`/`buildVerifyPrompt` に渡すよう変更
- 当初はクライアントが申告する設計で実装したが、`security-reviewer` エージェントのレビューで
  ①正規表現（英字のみ）だけでは LLM へのプロンプトインジェクションを防ぎきれない
  ②`extra_cards` のキャッシュキーが言語を含まないため、誤った言語で最初に生成されたカードが
  そのコースの全利用者に半永久的に配信され続ける、の2点（MEDIUM）を指摘され、**設計を変更**：
  クライアントからは送らせず、**Worker 自身が `courseId` からコースの `meta.json` を
  `env.ASSETS.fetch()` 経由で読んで学習言語・訳言語を決める**（isolate 内キャッシュ付き）。
  injection 経路とキャッシュ汚染の両方を構造的に排除できる
- 見出し語の許容文字（`HEADWORD_RE`）も英字限定から Unicode 文字全般
  （`/^[\p{L}][\p{L}\p{M}' -]{0,39}$/u`）に拡張——日本語（漢字・ひらがな・カタカナ）の
  見出し語も検索・生成できるように
- 日本語の活用（動詞・形容詞の語尾変化。例: 走る→走ります）で例文の完全一致チェックが
  誤って弾いていた不具合も同時に発見・修正（`headwordStemAppears()`：見出し語の末尾から
  最大3文字まで削った語幹を試す）

実地検証（`wrangler dev` + curl）で英語→日本語・日本語→タガログ語・タガログ語→日本語の
3方向すべて正常動作を確認済み（詳細は `.claude/plan-state.json` の `verified_facts` 参照）。
