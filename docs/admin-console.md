...
| 進捗（管理者向け） | **コース別の集計値だけ**を D1 に送る（回答履歴・例文は送らない） | 管理者に必要なのは「どれだけ進んだか」だけ。送る情報は最小にする |
| 進捗（端末移行用） | **単語ごとの学習状態を丸ごと** gzip 化して R2 に送る（本人のみ読み書き可・管理者は個別ダウンロードのみ） | 機種変更・PC→スマホの乗り換えで続きから再開するには集計値だけでは足りないため |
| 学習の一次データ | **これまで通り端末の IndexedDB**。D1・R2 はどちらもその写し | D1・R2 が両方消えても・オフラインでも・Worker が落ちても学習は 1 ミリも壊れない |
| 表示名 | サーバー → 端末の一方向。登録・変更は管理画面だけ | 端末側に編集 UI を持たせない（誰の端末か管理者が決められる） |
| 使えるコース | 管理画面で人ごとに割り当て。**未設定＝全コース** | メンバーに関係ないコースを並べない。取得できないときは全コース（学習を止めない） |

### 何が「削除」されるのか

| 操作 | Access 許可リスト | D1 の名簿 | 進捗データ | 元に戻せるか |
|---|---|---|---|---|
| **アクセス取消** | 削除 | `status='removed'` | **残る**（D1・R2とも） | 「再登録」で戻る |
| **完全削除** | 削除 | 行ごと削除 | **消える**（D1・R2とも） | 戻せない |
...
| メソッド | パス | 誰が | 何を |
|---|---|---|---|
| GET | `/api/me` | 全員 | 自分のメール・管理者かどうか・表示名・**使えるコース** |
...
| PATCH | `/api/admin/users` | 管理者 | 表示名・メモ・**使えるコース**の更新（送ったキーだけ更新する） |
...
- **利用者には「進捗が管理者に見えること」を伝える。** 管理画面から見えるのはコース別の集計値だけ
  （誰がどれだけ学習したか）。単語ごとの学習状態（R2）は本人の端末移行専用で、管理者は個別に
  ダウンロードしない限り中身を見ない。
