 ...
 * つまり日本語は5文字以上（10単位）で縮小対象に入る。全コースの実測では
 * ja-katakana 27.2%（「インタビュー」等の長い外来語）・ja-10-30k 15.9%・tl-0-2k 9.7%・
 * ja-3-10k 6.1%・ja-0-3k 2.4% が該当し、漢字1〜2字のコース（ja-kana / ja-kanji-*）は
 * ほぼ 0%。スマホ2列のタイル内寸は約149pxで、全角30pxのままだと6文字＝180pxで
 * はみ出すため、これらは縮小されるのが正しい挙動。
 */
