...
export type CourseId =
  ...
  | 'ja-kanji-advanced' // 上級漢字（JLPT N3〜N1・1974字。使用頻度10語以上＋tanos.co.uk N3/N2/N1完全収録の合併集合）
...
export interface Example {
  ...
  cloze?: string
  /**
   * この例文が Tatoeba 等の対訳コーパスでなく LLM 生成であることの明示フラグ（PLAN §3.4）。
   * true のときだけ出す（false/未生成の例文にはキー自体を付けない）。品質確認は人手ネイティブ
   * レビュー無し・複数AI相互検証のみの方針（判断ログ#18）のため、生成元をUIで区別できるように残す。
   */
  aiGenerated?: boolean
}
